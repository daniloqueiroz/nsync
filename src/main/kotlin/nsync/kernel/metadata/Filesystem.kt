...
import nsync.extensions.asString
...
internal class Filesystem(metadataDirectory: Path, private val fsId: String) {
    companion object CachedFactory {
        private val cache: MutableMap<String, Filesystem> = mutableMapOf()
        fun get(fsId: String) =
                cache.getOrPut(fsId, { Filesystem(Configuration.directory, fsId) })
    ...
    private val controlFile: AsyncFile<Index> = AsyncFile(
            metadataDirectory.resolve("$fsId.$CONTROL_FILE_SUFFIX"),
            Index
    )
    private val inodesFile: AsyncFile<Inode> = AsyncFile(
